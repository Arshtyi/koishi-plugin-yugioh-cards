...
此插件提供简单易用的游戏王卡片查询功能，包括随机抽卡、根据ID查询卡片等功能。
...
* \`ygo.query <cardIds>\` - 根据ID查询卡牌，支持查询多张卡牌（用空格分隔多个ID）。
...
### 随机抽卡
输入 \`ygo.random\` 即可随机抽取一张游戏王卡牌，机器人会返回卡片图片和详细信息，包括：
- 卡名
- 卡片ID
- 类型（怪兽Typeline/魔法/陷阱）
- 属性（对于怪兽卡）
...
### 查询卡片
输入 \`ygo.query <cardIds>\` 可以查询特定ID的卡片，例如：
* \`ygo.query 10000040\` - 查询ID为10000040的卡牌。
* \`ygo.query 10000040 10000080\` - 同时查询多张卡牌，结果将以转发消息形式发送。
...
* 如果查询不到某些卡片，可能是数据库中尚未包含该卡片，或者卡片ID输入有误。
